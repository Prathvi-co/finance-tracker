  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        ...
        if (!token) return;
        const config = { headers: { 'x-auth-token': token } };
        const res = await axios.get(`http://localhost:5000/api/transactions?page=${page}&limit=${limit}`, config);
    ...
    const fetchSummary = async () => {
      try {
        ...
        if (!token) return;
        const config = { headers: { 'x-auth-token': token } };
        const res = await axios.get('http://localhost:5000/api/transactions/summary', config);
        setSummary(res.data);
      } catch (err) {
